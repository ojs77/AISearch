...
def alg_a_enhanced(dist_matrix):  
    class Salesman:
        def __init__(self, starting_city):
            # The salesman starts at a specified city
            self.starting_city = starting_city
            self.cities_visited = [self.starting_city]
            self.tour_distance = 0
            self.current_city = self.starting_city

        def tour(self, dist_matrix):
            while len(self.cities_visited) < len(dist_matrix):
                # Finds max distance from current city to next city
                min_dist_matrix = max(dist_matrix[self.current_city])
                # finds min distance to next city that the salesman hasn't already been to
                for i in range(len(dist_matrix[self.current_city])):
                    if dist_matrix[self.current_city][i] < min_dist_matrix and i not in self.cities_visited and dist_matrix[self.current_city][i] != 0:
                            min_dist_matrix = dist_matrix[self.current_city][i]
                            new_city = i
                # adds this new city to the tour
                self.tour_distance += min_dist_matrix 
                self.current_city = new_city
                self.cities_visited.append(self.current_city)
            # adds distance from final city to first city to the tour distance
            self.tour_distance += dist_matrix[self.cities_visited[-1]][self.starting_city]
            
            return self.cities_visited, self.tour_distance
    ...
    # Enhancement:
    n = len(dist_matrix)
    # Create n salesmen that each start at different cities and store them in a list
    salesmen = list()
    for i in range(n):
        salesmen.append(Salesman(i))

    for salesman in salesmen:
        # create tour for each salesman
        tour, tour_length = salesman.tour(dist_matrix)
        # evaluate which salesman has the shortest tour
        if tour_length < best_tour_length or best_tour_length == 0:
            best_tour = tour
...
tour, tour_length = alg_a_enhanced(dist_matrix)
